...
/**
 * Detects all matches of 3 or more gems on the board.
 *
 * The algorithm works in 4 steps:
 * 1. Scan each row for horizontal runs of 3+ same-colored gems
 * 2. Scan each column for vertical runs of 3+ same-colored gems
 * 3. Merge overlapping runs of the same color (this detects L and T shapes)
 * 4. Classify each merged group to determine what special gem it creates
 *
 * This is the most important algorithm in the game — it determines
 * when gems should be cleared and what special gems to create.
 */
class MatchDetector {

    /**
     * A run of consecutive same-colored gems in a line.
     * @param positions The positions in this run (in order)
     * @param gemType The color of the gems
     ...
     * @return A list of MatchResult objects, each representing a group of matched gems.
    ...
    /**
     * Step 1: Find all horizontal runs of 3+ same-colored gems.
     *
     * For each row, we scan left to right and track consecutive gems
    ...
    private fun findHorizontalRuns(board: BoardState): List<Run> {
        ...
        for (row in 0 until board.rows) {
            var runStart = 0
            var currentType: GemType? = null

            for (col in 0 until board.cols) {
                val gem = board.grid[row][col]

                if (gem != null && gem.type == currentType) {
    ...
    /**
     * Step 2: Find all vertical runs of 3+ same-colored gems.
    ...
    /**
     * Step 3: Merge overlapping runs of the same gem type.
     *
     * Two runs should be merged if:
     * - They have the same GemType
        ...
        while (merged) {
            ...
            for (i in groups.indices) {
                for (j in i + 1 until groups.size) {
                    if (groups[i].gemType == groups[j].gemType &&
     ...
     * - pivotPosition: Where a special gem should be placed
     */
     ...
     * For a straight line of 3: returns the center gem.
     * For a line of 4: returns the 2nd gem.
     * For a line of 5: returns the 3rd gem.
     */
